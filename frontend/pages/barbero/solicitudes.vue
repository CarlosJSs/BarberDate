...
export default {
  ...
  watch: {
    solicitudes: {
      handler (newSolicitudes) {
        this.soliNegadas = newSolicitudes.filter(
          solicitud => solicitud.status === 'denied'
        )
        this.soliPendientes = newSolicitudes.filter(
  ...
  methods: {
    loadMisSolicitudes () {
      ...
      this.$axios
        .get(`/citas/barbero/${this.barberoID}`, {
          headers: {
